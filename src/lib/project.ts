...
async function checkInternetConnectivity(): Promise<boolean> {
  ...
  for (const endpoint of httpEndpoints) {
    try {
      ...
      await execAsync(`curl -s --max-time 10 --head ${endpoint}`, {
        timeout: 15000,
      });
...
export async function initializeSupabaseCore() {
  ...
  try {
    ...
    if (!coreExists) {
      const repoUrl =
        process.env.SUPABASE_CORE_REPO_URL ||
...
export async function createProject(
  ...
  try {
    ...
    const containerMappings = [
      ...
      {
        original: "realtime-dev.supabase-realtime",
        replacement: `realtime-dev.${slug}-realtime`,
      },
      ...
      { original: "supabase-meta", replacement: `${slug}-meta` },
      {
        ...
        replacement: `${slug}-edge-functions`,
      },
...
export async function deployProject(projectId: string) {
  try {
    ...
    if (!checks.docker) {
      throw new Error(
        "Docker is not installed or not running. Please install Docker Desktop and ensure it is started before deploying."
      );
    }

    if (!checks.dockerCompose) {
      throw new Error(
        "Docker Compose is not available. Please ensure Docker Desktop includes Docker Compose or install it separately."
      );
    }
    ...
    try {
      ...
      if (checks.internetConnection) {
        ...
        try {
          ...
          console.warn(
            "Failed to pull some images, will try to use existing/cached images:",
            pullError
          );
        ...
        console.warn(
          "No internet connectivity detected, using cached Docker images"
        );
      }
      ...
      if (errorMessage.includes("maxBuffer length exceeded")) {
        throw new Error(
          'Docker deployment generated too much output. This usually means the deployment is working but Docker is downloading many large images. Please wait a few more minutes and check Docker Desktop to see if containers are starting. You can also try running "docker compose up -d" manually in the project directory.'
        );
      } else if (
    ...
    try {
      ...
      console.log(
        `Deployment successful: ${runningContainers.length} containers running`
      );
    } catch {
      console.warn(
        "Could not verify container status, but deployment may have succeeded"
      );
    }
...
export async function deleteProject(projectId: string) {
  try {
    ...
    try {
      ...
      console.warn(
        "Failed to stop Docker containers (they may not be running):",
        dockerError
      );
